...
Claimable Neon is the successor to the service behind [neon.new](https://neon.new). An AI agent
gets a temporary Lakebase Postgres database on Neon without a human signing up first. A human can
later claim that project into a Neon organization.

The agent never receives a Neon API key. It receives an [`auth.md`](https://auth.md) identity
assertion, exchanges it for a short-lived access token, and this service sits on every authorized
call. The public contract is in the [README](../README.md). What is actually implemented is in
[`status.md`](status.md).

Target origin: `https://claimable.neon.tech/v1`. The service is not deployed yet; neon.new remains
the live product.
...
|---|---|---|
| Create | `POST https://neon.new/api/v1/database` with `{ "ref": "…" }` | `POST /v1/agent/identity` (`auth.md`) |
...
| After claim | `GET /api/v1/database/{id}` returns `connection_string: null` | Status poll goes `pending` → `accepted` → `reconciled`. Only `reconciled` means the assertion is revoked and the ceremony finished |
...
| | Responsibility |
|---|---|
| [PR #1](https://github.com/neondatabase/claimable-neon/pull/1) | Capability/scope wire format (`postgres`, `data_api`, `auth`, `storage`, `functions`, `ai_gateway`; scopes are always `<capability>.<action>`). Registration must not create a transfer. Seeded CI (Actions is still disabled at the org). Vitest advisory bump. |
| [15e690b](https://github.com/neondatabase/claimable-neon/commit/15e690b) | The running service: registration, tokens, provisioning, credentials, proxy, local E2E. |
| [91483f4](https://github.com/neondatabase/claimable-neon/commit/91483f4) | Auth provisioning and claim-status polling. |
| [8c625fc](https://github.com/neondatabase/claimable-neon/commit/8c625fc) | CLI Postgres connections through claimable tokens. |
| [c739741](https://github.com/neondatabase/claimable-neon/commit/c739741) | Pre-transfer credential teardown and the recorded claim ceremony. |
| [9c62939](https://github.com/neondatabase/claimable-neon/commit/9c62939) | Close races between password rotation and live sessions at claim time. |
| [PR #2](https://github.com/neondatabase/claimable-neon/pull/2) | `usage_events` + `track.neon.tech`. Reconcile under a reserved postgres.js connection so the status poll can finish after the human accepts. |

Still open: Neon Function deployment, a dedicated `track.neon.tech` write key, and automatic
