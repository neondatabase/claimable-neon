...
| Pre-transfer credential teardown and accepted-to-reconciled transition | `lib/claims/reconcile.ts` | recorded in `test/e2e/website-and-claim.test.ts`; two-org live claim of `calm-feather-04833677` into `org-summer-dust-66593634` (2026-08-25). Transfer into free Testing `org-old-flower-82714815` returned 406 (plan requirements: holding org is `agent_free`) |

## Not yet implemented

- Automatic deletion of expired unclaimed projects. Orbit task 101 on project 6 (Neon AX/DX), blocked on Neon Functions cron.
- Dedicated Neon service user whose only organization is the unclaimed-projects org. Stand-in: account API key for `andre.landgraf@databricks.com` (`claimable-fn-20260825`). Unclaimed projects provision into `org-icy-firefly-99910178` (🚨 Prod Claimable Neon Projects, `agent_free`). Function compute is Neon Prod `soft-morning-58679842`. Do not use an org API key.
